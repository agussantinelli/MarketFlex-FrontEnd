...
describe('UsersListView Component', () => {
    ...
    it('fetches and renders users on mount', async () => {
        (ManagementService.getUsers as any).mockResolvedValue(mockUsersResponse);

        render(<UsersListView />);

        // Check if service was called
        await waitFor(() => {
            expect(ManagementService.getUsers).toHaveBeenCalled();
        });
    ...
    it('shows empty state when no users found', async () => {
        (ManagementService.getUsers as any).mockResolvedValue({
            status: 'success',
            data: [],

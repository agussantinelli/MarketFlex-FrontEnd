...
import type { ManagementUser, PaginatedResponse } from '../../types/management\.types';
...
export default function UsersListView() {
    const [usersData, setUsersData] = useState<PaginatedResponse<ManagementUser> | null>(null);
    const [loading, setLoading] = useState(true);
    const [page, setPage] = useState(1);
    const [search, setSearch] = useState('');
    const [sort, setSort] = useState('newest');
    const [selectedUserForPurchases, setSelectedUserForPurchases] = useState<ManagementUser | null>(null);

    const fetchUsers = async () => {
        if ((window as any).showManagementLoader) (window as any).showManagementLoader();
        setLoading(true);
        const data = await ManagementService.getUsers(page, 10, search, sort);
        if (!data) {
    ...
    const handleEdit = (user: ManagementUser) => {
        window.location.href = `/management/users/${user.id}`;
    };
    ...
    const handleDelete = (user: ManagementUser) => {
        (window as any).showDeleteUserModal(async () => {
            try {
                ...
                const result = await ManagementService.deleteUser(user.id);
    ...
    const columns: Column<ManagementUser>[] = [
        ...
        {
            header: 'Nombre',
            accessor: (user: ManagementUser) => <span style={{ fontWeight: 500, color: 'var(--text-main)' }}>{user.nombre} {user.apellido}</span>,
        },
        {
            header: 'Email',
            accessor: (user: ManagementUser) => <span style={{ color: 'var(--text-muted)' }}>{user.email}</span>,
        },
        {
            header: 'Rol',
            accessor: (user: ManagementUser) => (
        ...
        {
            header: 'Registro',
            accessor: (user: ManagementUser) => <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>{new Date(user.creadoEn).toLocaleDateString()}</span>,
        },
        {
            header: 'Compras',
            accessor: (user: ManagementUser) => (
            ...
            <header className={styles.header}>
                <div className={styles.titleSection}>
                    <div className={dashboardStyles.dashboardHeader}>
                        ...
                        <a href="/management/users/new" className={dashboardStyles.createButton} style={{ textDecoration: 'none' }}>

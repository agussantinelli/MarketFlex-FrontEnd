...
describe('CreateUserView Component', () => {
    ...
    it('calls ManagementService.createUser on valid submit', async () => {
        (ManagementService.createUser as any).mockResolvedValue({ status: 'success' });
        ...
        await waitFor(() => {
            expect(ManagementService.createUser).toHaveBeenCalled();
            expect(mockTriggerSileo).toHaveBeenCalledWith('success', expect.stringContaining('éxito'));
        }, { timeout: 4000 });
    });

    it('handles server errors gracefully', async () => {
        (ManagementService.createUser as any).mockResolvedValue({ status: 'error', message: 'DNI ya existe' });

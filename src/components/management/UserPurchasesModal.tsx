...
const UserPurchasesModal: React.FC<UserPurchasesModalProps> = ({ userId, userName, onClose }) => {
    const [purchases, setPurchases] = useState<ManagementPurchase[]>([]);
    ...
    useEffect(() => {
        const fetchPurchases = async () => {
            try {
                const data = await ManagementService.getUserPurchases(userId);
                setPurchases(data || []);
            } catch (error) {
    ...
    return (
        <div className="management-modal-overlay" style={{
            position: 'fixed',
            top: 0,
            left: 0,
            width: '100%',
            ...
            <div className="management-modal-content" style={{

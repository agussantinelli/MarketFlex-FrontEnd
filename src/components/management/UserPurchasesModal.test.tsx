...
describe('UserPurchasesModal Component', () => {
    ...
    it('fetches and renders purchases on mount', async () => {
        (ManagementService.getUserPurchases as any).mockResolvedValue(mockPurchases);
        ...
        await waitFor(() => {
            expect(ManagementService.getUserPurchases).toHaveBeenCalledWith('u-1');
    ...
    it('shows empty state when no purchases', async () => {
        (ManagementService.getUserPurchases as any).mockResolvedValue([]);
    ...
    it('triggers Sileo error notification when fetch fails', async () => {
        (ManagementService.getUserPurchases as any).mockRejectedValue(new Error('Network Error'));

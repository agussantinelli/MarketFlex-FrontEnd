...
const EditProductView: React.FC<EditProductViewProps> = ({ productId }) => {
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                ...
                const prodRes = await ManagementService.getManagementProduct(productId);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await ManagementService.updateProduct(productId, payload);
            ...
            if (result.status === 'success') {
                ...
                setTimeout(() => {
                    window.location.href = `/management/products/${productId}`;
                }, 1500);
            } else {
                if ((window as any).triggerSileo) {
    ...
    return (
        <div style={{ padding: '0 1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2rem' }}>
                <a
                    href={`/management/products/${productId}`}
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1.25rem', marginTop: '1rem', paddingTop: '2rem', borderTop: '1px solid rgba(255, 255, 255, 0.05)' }}>
                    <a href={`/management/products/${productId}`} className={styles.btnSecondary} style={{ textDecoration: 'none', padding: '0.8rem 2rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)' }}>
                        Cancelar
                    </a>

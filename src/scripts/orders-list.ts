...
export async function initOrdersList(styles: Record<string, string>) {
    ...
    try {
        ...
        ordersList.innerHTML = response.data.map((order: Purchase) => {
            ...
            return `
                <a href="/orders/${order.id}" class="${styles.orderCard}">
                    <div class="${styles.orderIconWrapper}">
                    ...
                    <div class="${styles.viewDetailBtn}">
                        Ver Detalle
        ...
        ordersLoading.innerHTML = `
            <p class="${styles.emptyText}" style="color: #ef4444;">No pudimos cargar tus compras. Por favor, reintenta más tarde.</p>
        `;
    }
}

...
test.describe('MarketFlex Foundational Flow', () => {
    ...
    test('should complete the foundational flow: Login -> Find Product -> View Detail -> Cart -> Checkout', async ({ page }) => {
        ...
        await expect(submitPurchaseBtn).toBeVisible({ timeout: 10000 });
        ...
        await expect(page.locator('h1').getByText(/¡Compra Exitosa!/i)).toBeVisible({ timeout: 10000 });
        ...
        await expect(authUserArea).toBeVisible({ timeout: 15000 });

        const userDropdownBtn = page.locator('.dropdown-trigger').first();
        await expect(userDropdownBtn).toBeVisible({ timeout: 10000 });
        ...
        await expect(page.locator('h1').getByText(/Mis Compras/i)).toBeVisible({ timeout: 10000 });

        console.log('Opening the latest order...');
        const firstOrderLink = page.locator('a[href^="/orders/"]').first();
        await expect(firstOrderLink).toBeVisible({ timeout: 15000 }); // Wait for orders to load from API
        await firstOrderLink.click();

        console.log('Verifying Order Detail Page...');
        await expect(page).toHaveURL(/\/orders\//, { timeout: 15000 });
        await expect(page.locator('h2').getByText(/Artículos/i)).toBeVisible({ timeout: 10000 });
